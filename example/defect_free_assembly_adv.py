"""Demo: geometry-driven RIPA assembly on the same grid as the AOD example.
...
Outputs are written to `render/`:
    - `defect_free_assembly_adv_t0.png`
    - `defect_free_assembly_adv_tfinal.png`
    - `defect_free_assembly_adv.gif`
    - `defect_free_assembly_adv_benchmark.png`
"""
...
OUT_DIR = ROOT / "render"
PREFIX = "defect_free_assembly_adv"
...
def render_adv_outputs(sequence: Sequence, request: RoutingRequest) -> None:
    ...
    render_check_outputs(
        ...
        view="demo",
        title_prefix=f"RIPA pebble adv defect-free assembly - target {TARGET_SIDE}x{TARGET_SIDE}",
        gif_fps=6,
...
def main() -> None:
    ...
    adv = next(result for result in results if result.name == "ripa_pebble_adv")
    if not adv.ok or adv.sequence is None:
        raise RuntimeError("ripa_pebble_adv did not produce a valid assembly")
    assert_request_satisfied(adv.sequence, request)
    ...
    if not args.no_render:
        ...
        render_adv_outputs(adv.sequence, request)
