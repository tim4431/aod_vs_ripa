"""C++ CCBS demo for a 6x6 RIPA x-inversion.
...
times out with the current exact backend.  This demo uses C++ CCBS to plan each
pair swap, then launches geometrically independent swaps together in async
column-parity layers. The final sequence is validated with the RIPA bang-bang
...
def main() -> None:
    ...
    layers = len({stat.layer for stat in stats})
    print(
        f"N={N}, storage_period={STORAGE_PERIOD}, "
        f"atoms={len(src)} ({TARGET_SIDE}x{TARGET_SIDE}), "
        "task=x-inversion with C++ CCBS async layers"
    )
    print(
        f"total_us={sequence.total_duration() * 1e6:.3f}, "
        f"layers={layers}, stages={len(stats)}, "
        f"segments={sum(len(atom.segments) for atom in sequence.ensemble.atomtrajs)}, "
        f"expanded={sum(stat.high_level_expanded for stat in stats)}, "
        f"generated={sum(stat.high_level_generated for stat in stats)}"
    )
    ...
    render_animation(
        ...
        title=f"C++ CCBS x-inversion ({TARGET_SIDE}x{TARGET_SIDE} on {N}x{N})",
    )
