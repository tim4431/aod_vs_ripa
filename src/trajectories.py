...
def make_segment(start: tuple[int, int],
                 end: tuple[int, int],
                 start_time: float,
                 a_max: float,
                 *,
                 duration: Optional[float] = None,
                 channel: Optional[Channel] = None) -> Trajectory:
    ...
    return Trajectory(start_time=start_time, duration=T,
                      start_pos=tuple(map(int, start)),
                      end_pos=tuple(map(int, end)),
                      fn=fn, channel=channel)


def make_hold(pos: tuple[int, int], start_time: float, duration: float,
              *, channel: Optional[Channel] = None) -> Trajectory:
    """Stay-put segment. Useful for forced waits."""
    p = (int(pos[0]), int(pos[1]))
    return Trajectory(start_time=start_time, duration=duration,
