...
`UnlabeledRIPAPebbleAdvScheduler` wraps `RIPAPebbleAdvScheduler` for set→set requests with two routing modes: a staged unlabeled compaction (default, `unlabeled_assignment="min_sum"`) that picks moves greedily from the current occupancy without committing to a fixed pairing, and an assignment-based path (`unlabeled_assignment="min_max"` or when staged compaction is disabled) that solves a Hungarian/bottleneck assignment on path-cost estimates and delegates the resulting labeled problem to the inner scheduler.
