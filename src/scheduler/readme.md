...
`RIPACCBSCScheduler` uses the shared RIPA CCBS adapter (`ripa_ccbs_common.py`) to build the route graph and emit validated RIPA trajectories, then hands the labeled CCBS search instance to the C++ backend. It no longer imports the old Python CCBS reference solver.
