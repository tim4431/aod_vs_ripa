...
def draw_atoms(ax, ensemble: AtomEnsemble, t: float, *,
    ...
    xy = ensemble.xy_at(t)
    held_set = set(int(k) for k in (held if held is not None
                                     else _held_atom_indices(ensemble, t)))
    ...
    if not fast and held_set:
        ...
        ax.scatter(held_xy[:, 0], held_xy[:, 1],
                   s=size * 6, c="red", alpha=0.18, zorder=2)
...
def draw_motion_blur(ax, ensemble: AtomEnsemble, t: float, *,
                     window: float = 5e-6, alpha: float = 0.25):
...
def draw_planned_trajectories(ax, ensemble: AtomEnsemble, t: float, *,
                              n_samples: int = 30, alpha: float = 0.4):
    ...
    for atom in ensemble.atoms:
        for seg in atom.segments:
            ...
            pts = (pts - c) * grid.d
            ax.plot(pts[:, 0], pts[:, 1], "--", color="0.4",
                    alpha=alpha, lw=0.8, zorder=1)


# --- Frequency panels --------------------------------------------------------

def _active_segments(ensemble: AtomEnsemble, t: float, channel: str) -> list[Trajectory]:
    """Currently-active segments of a given channel ('row' or 'col')."""
    out: list[Trajectory] = []
    for atom in ensemble.atoms:
        for seg in atom.segments:
            if seg.channel == channel and seg.start_time <= t <= seg.end_time and seg.duration > 0:
                out.append(seg)
    return out
...
def draw_tone_trajectories(ax, ensemble: AtomEnsemble, channel: str, *,
                           spec: RIPASpec, t: float, n_samples: int = 50):
...
def _render_frame(args):
    ...
    if spec is not None:
        ...
        row_active = {atom.atom_id for atom in ensemble.atoms
                      for seg in atom.segments
                      if seg.channel == "row" and seg.start_time <= t <= seg.end_time and seg.duration > 0}
        col_active = {atom.atom_id for atom in ensemble.atoms
                      for seg in atom.segments
                      if seg.channel == "col" and seg.start_time <= t <= seg.end_time and seg.duration > 0}
        row_pos = [ensemble.atoms[k].position_at(t) for k in row_active]
        col_pos = [ensemble.atoms[k].position_at(t) for k in col_active]
        draw_tone_stems(ax_row, [nu_row(i, j, spec) for i, j in row_pos], FSR1=spec.FSR1)
        draw_tone_stems(ax_col, [nu_col(i, j, spec) for i, j in col_pos], FSR1=spec.FSR1)
...
def animate(seq: Sequence, opts: AnimationOptions = AnimationOptions()) -> str:
    ...
    opts_dict = dict(fast=opts.fast, show_planned=opts.show_planned,
                     spec=spec_dict, static_traps=tuple(opts.static_traps))
