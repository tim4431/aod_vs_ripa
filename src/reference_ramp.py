...
Each connector exposes the value plus first and second derivatives, so traps
can ask the ramp for `velocity_at(t)` and `acceleration_at(t)` without finite
differences.
"""
...
class PolynomialConnector:
    ...
    def second_derivative(self, u: ArrayLike) -> NDArray[np.float64]:
        """Second derivative of the shape function with respect to `u`."""
...
class RampSequence:
    ...
    values and derivative queries return zero.
    """
    ...
    def acceleration_at(self, time_s: float) -> NDArray[np.float64]:
        """Return `d²center/dt²` at `time_s`. Outside the table this is zero."""

        index, u = self._segment_index_and_fraction(time_s)
        if index is None:
            return np.zeros(3, dtype=float)
        dt = float(self.times_s[index + 1] - self.times_s[index])
        curvature = float(self.position_profile.second_derivative(u)) / (dt * dt)
        c0 = self.centers_m[index]
        c1 = self.centers_m[index + 1]
        return curvature * (c1 - c0)
    ...
    def depth_acceleration_at(self, time_s: float) -> float:
        """Return `d²depth/dt²` at `time_s` in uK/s²."""

        index, u = self._segment_index_and_fraction(time_s)
        if index is None:
            return 0.0
        dt = float(self.times_s[index + 1] - self.times_s[index])
        curvature = float(self.depth_profile.second_derivative(u)) / (dt * dt)
        d0 = float(self.depths_uK[index])
        d1 = float(self.depths_uK[index + 1])
        return curvature * (d1 - d0)
